...
export function Board() {
  ...
  const { 
    ...
    validMoves, score, handleMove, resetGame 
  } = useReversi(difficulty);
      ...
      <div className={styles.board} role="grid" aria-label="Reversi Board">
        {board.map((row, r) => (
          row.map((cell, c) => {
            ...
            const isClickable = !isGameOver && currentPlayer === PLAYER && !isThinking && isValidMove;

            return (
              <div
                key={`${r}-${c}`}
                className={`${styles.cell} ${isClickable ? styles.validMove : ''}`}
                onClick={() => {
